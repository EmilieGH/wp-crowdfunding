...
if (! class_exists('Wpneo_Crowdfunding')) {
    ...
    class Wpneo_Crowdfunding{
                ...
                for ($i=0; $i<$total_update_field; $i++){
                    if (! empty($wpneo_prject_update_title_field[$i])) {
                        $data[] = array(
                            ...
                            'details'   => sanitize_text_field($wpneo_prject_update_details_field[$i])
                        );
                    }
                }
        ...
        function wpneo_donate_input_field()
        {
            ...
            if ($product->get_type() == 'crowdfunding') {
                ...
                if (WPNEOCF()->campaignValid()) {
                    ...
                    $html .= apply_filters('neo_donate_field', '<input type ="number" class="input-text amount wpneo_donation_input text" name="wpneo_donate_amount_field" placeholder="' . get_woocommerce_currency_symbol() . '" min="0" value="'.esc_attr($recomanded_price).'" />');
        ...
        function wpneo_save_user_donation_to_cookie( $array, $int ) {
            if ($array['data']->get_type() == 'crowdfunding'){
                if ( ! empty($_POST['wpneo_donate_amount_field'])){
                    ...
                    if ( ! empty($_POST['wpneo_rewards_index'])){
                        ...
                        WC()->session->set('wpneo_rewards_data', array('rewards_index' => $wpneo_rewards_index, 'product_id' => $product_id, '_cf_product_author_id' => $_cf_product_author_id) );
                    }else{
        ...
        public function check_anonymous_backer(){
            ...
            echo '<label><input type="checkbox" value="true" name="mark_name_anonymous" /> '.__('Mark My Name anonymous', 'wp-crowdfunding').' </label>';
            echo '</div>';
        }
